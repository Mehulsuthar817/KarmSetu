...
const STATUS_STYLES = {
  pending: {
    pill: "bg-amber-500/20 border-amber-400/30 text-amber-200",
    dot: "bg-amber-400",
    label: "Pending",
  },
  reviewed: {
  ...
  accepted: {
    pill: "bg-emerald-500/20 border-emerald-400/30 text-emerald-200",
    dot: "bg-emerald-400",
    label: "Accepted",
  },
  rejected: {
    pill: "bg-red-500/20 border-red-400/30 text-red-300",
    dot: "bg-red-400",
    label: "Rejected",
  },
};
...
function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`flex items-center gap-1.5 px-3 py-1.5 border rounded-full text-xs font-semibold uppercase tracking-widest ${s.pill}`}
    >
        ...
        return (
          <div key={step} className="flex items-center flex-1">
            <div className="flex flex-col items-center gap-1">
              ...
              />
              <span className="text-gray-600 text-[10px] font-medium capitalize hidden sm:block">
                {step}
              </span>
            </div>
            {!isLast && (
        ...
        <motion.div {...fadeUp(0.09)} className="flex gap-2 flex-wrap mb-6">
          ...
          {Object.entries(STATUS_STYLES).map(([key, val]) => (
            ...
            >
              {val.label} (
              {
                applications.filter((a) => a.status?.toLowerCase() === key)
                  .length
              }
              )
            </button>
